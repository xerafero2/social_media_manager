...
class _ExportImportScreenState extends State<ExportImportScreen> {
  ...
  Future<List<Map<String, dynamic>>> _accountsFromJsonString(String jsonStr) async {
    final List<dynamic> decoded = jsonDecode(jsonStr);
    return decoded.map((e) => Map<String, dynamic>.from(e)).toList();
  }

  Future<String> _accountsToCsvString() async {
    final accounts = await DatabaseHelper.instance.getAllAccounts();
    ...
    List<List<dynamic>> rows = [columns];
    for (var acc in accounts) {
      rows.add(columns.map((c) => acc[c] ?? '').toList());
    }
    return const ListToCsvConverter().convert(rows);
  }

  Future<List<Map<String, dynamic>>> _accountsFromCsvString(String csv) async {
    ...
    return dataRows.map((row) {
      ...
      for (int i = 0; i < headers.length && i < row.length; i++) {
        acc[headers[i]] = row[i].toString();
      }
      return acc;
    }).toList();
  }

  String _accountsToRawText(List<Map<String, dynamic>> accounts) {
    if (accounts.isEmpty) return 'Tidak ada data.';
    final buffer = StringBuffer();
    for (int i = 0; i < accounts.length; i++) {
      final a = accounts[i];
      buffer.writeln('=== Akun ${i + 1} ===');
      buffer.writeln('Platform/Nama: ${a['name'] ?? '-'}');
      buffer.writeln('Username/Email: ${a['identifier']}');
      buffer.writeln('Password: ${a['password']}');
      buffer.writeln('2FA: ${a['a2f'] == 1 ? "Aktif" : "Nonaktif"}');
      buffer.writeln('Secret Key: ${a['secret_key'] ?? '-'}');
      buffer.writeln('Tanggal Lahir: ${a['dob'] ?? '-'}');
      buffer.writeln('Tahun Akun: ${a['account_year'] ?? '-'}');
      buffer.writeln('Tags: ${a['tags'] ?? '-'}');
      buffer.writeln('Dibuat: ${a['created_at'] ?? '-'}');
      buffer.writeln('Diperbarui: ${a['updated_at'] ?? '-'}');
      buffer.writeln();
    }
  ...
  Future<void> _importFromFile(String extension) async {
    try {
      ...
      if (content.isEmpty) {
        _showSnack('File kosong');
        return;
      }
      List<Map<String, dynamic>> accounts;
      if (extension == 'json') {
        accounts = await _accountsFromJsonString(content);
      ...
      body: SingleChildScrollView(
        ...
        child: Column(
          ...
          children: [
            ...
            const Divider(height: 32),
            _sectionTitle('Impor Data'),
            _importButton('Impor dari JSON', Icons.upload_file, () => _importFromFile('json')),
            _importButton('Impor dari CSV', Icons.upload_file, () => _importFromFile('csv')),
            _importButton('Impor dari String Terenkripsi', Icons.lock_open, _importFromEncryptedString),
          ],
        ),
      ),
    );
  ...
  Widget _exportButton(String label, IconData icon, VoidCallback onPressed) {
    return Padding(
  ...
  Widget _importButton(String label, IconData icon, VoidCallback onPressed) {
    return Padding(
      padding: const EdgeInsets.only(bottom: 10),
...
class _AccountCardState extends State<AccountCard> {
  ...
  void _copyToClipboard(String text, String label) {
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(content: Text('$label tersalin', style: const TextStyle(fontWeight: FontWeight.w500)), duration: const Duration(seconds: 1)),
    );
  }
